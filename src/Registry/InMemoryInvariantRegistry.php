...
final class InMemoryInvariantRegistry implements InvariantRegistry
{
    ...
    public function register(string|Invariant $invariant): void
    {
        ...
        $key = $instance::key();
        if (isset($this->invariants[$key]) && $this->invariants[$key]::class !== $instance::class) throw new InvalidArgumentException("Duplicate invariant key [{$key}].");
        $this->invariants[$key] = $instance;
    }
    public function get(string $key): Invariant
    {
        if (! isset($this->invariants[$key])) throw new InvalidArgumentException("Invariant [{$key}] is not registered.");
